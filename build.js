...
var minimistOpts = { 
  string: ['target'],
  boolean: ['clean', 'cmake', 'make', 'run', 'build', 'rebuild', 'test', 'publish', 'debug', 'release'],
  default: {
    target: '?'
  },
  unknown: function(x) { 
...
function publish() {
  ...
  // sudo openocd -f interface/cmsis-dap.cfg -f target/rp2040.cfg -c "adapter speed 5000" -c "program pico-flash-multicore-0.0.1.elf verify reset exit"
